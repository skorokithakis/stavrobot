...
export async function escalatingSummarize(
  ...
  log.info(`[solonbot] Compaction level 1 failed (textLength=${level1Text.length}, input=${inputLength}, stopReason=${level1Response.stopReason}, nonTextBlocks=${level1NonTextBlocks}), attempting level 2 bullet-point summary (target: ${targetTokens} tokens).`);
  ...
  log.info(`[solonbot] Compaction level 2 failed (textLength=${level2Text.length}, input=${inputLength}, stopReason=${level2Response.stopReason}, nonTextBlocks=${level2NonTextBlocks}), falling back to level 3 truncation.`);
